...
class GraphProvider implements DataSource<RPCData> {
  ...
  /**
   * The flag to indicate if it should poll continuously or stop after first fetch
   */
  ...
  async * read (height: bigint): AsyncGenerator<RPCData> {
    ...
    */
    // Reset block height everytime this function is called
    let startBlock = height
    const endBlock = 'latest'
    while (true) {
      console.log('GraphProvider: Fetching for range ', startBlock, endBlock)
      for await (const event of this.from({ startBlock, endBlock })) {
        // console.log('FoundEvent', event)
        if (!this.syncing) { break }
        // Update the block up to which the data is synced
        startBlock = BigInt(event.blockNumber)
        yield event
      }
      if (!this.isPollingSource) {
        break
      } else {
        // if it is a polling source, then it should wait for few minute here and start fetching the data again.
        // @TODO fix me
   ...
   */
  async * from (options: { startBlock: bigint, endBlock: bigint | 'latest' }): AsyncGenerator<RPCData> {
